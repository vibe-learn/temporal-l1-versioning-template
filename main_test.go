...
func TestTaskQueueDefault(t *testing.T) {
	if os.Getenv("TEMPORAL_TASK_QUEUE") == "" && TaskQueue() != "lesson-l1_versioning-tq" {
		t.Errorf("TaskQueue() default = %q, want lesson-l1_versioning-tq", TaskQueue())
	}
}
...
func TestOrderWorkflow(t *testing.T) {
	...
	env.OnActivity(StepOne, mock.Anything).Return("ok", nil)
	env.OnActivity(StepTwo, mock.Anything).Return("ok", nil)
...
func TestIntegration(t *testing.T) {
	...
	// поведения урока «Версионирование workflow».
}
